<?php
	...
	class Account extends Model{
		...
		public function verifyAdminAccount($uname,$pass){
		    ...
		    if($query->rowCount() > 0){
		    		if($result["sysStatus"] <> 0){return 2;}
		...
		public function registerUser($fname,$lname,$email,$phone,$password,$state,$account,$referal,$transpin){
	    	...
	    	if($email<>""){$queryC->bindParam(':e',$email,PDO::PARAM_STR);}
	     	...
	     	$queryC->execute();
	          ...
	          if($result["sPhone"] == $phone){$data=3; }
	          if($email<>""){if($result["sEmail"] == $email){$data=2; }}
	          if($result["sEmail"] == $email && $result["sPhone"] == $phone){$data=1; }
	    	...
	    	$queryC->bindParam(':ph',$phone,PDO::PARAM_STR);
	     	...
	     	$queryC->execute();
	      	...
	      	if($queryC->rowCount() > 0){
				...
				if($result->sRegStatus == 1){return 2;}
